...
To position the cells, we will use a [`(UI.Area)`](https://docs.fragcolor.xyz/docs/shards/UI/Area/). By default it is anchored at the top left corner, which means the position at that corner is `(float2 0 0)`.
...
=== "EDN"
    ...
    5. [`(UI.Area)`](https://docs.fragcolor.xyz/docs/shards/UI/Area/) requires a `(float2)` type for its `:Position` parameter, so we do a conversion using [`(ToFloat2)`](https://docs.fragcolor.xyz/docs/shards/General/ToFloat2/)
    ...
    9. In that action, we [`(Match)`](https://docs.fragcolor.xyz/docs/shards/General/Match/) the value of the cell to the corresponding character we have chosen.
    10. Then the matched character is displayed in place of that grid element using [`(UI.Label)`](https://docs.fragcolor.xyz/docs/shards/UI/Label/).
...
=== "EDN"
    ...
    3. [`(Slice)`](https://docs.fragcolor.xyz/docs/shards/General/Slice/) gives a part of a sequence in a range. `1` means we start at the second element of the sequence (in other words, we skip `1` element), and `-1` means we stop at one element before the last (in other words, we skip `1` element from the end).
    4. [`(RTake)`](https://docs.fragcolor.xyz/docs/shards/General/RTake/) is similar to [`(Take)`](https://docs.fragcolor.xyz/docs/shards/General/Take/), except it starts from the end of the sequence instead of the beginning (i.e. "reverse take").
